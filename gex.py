# This cell will generate the requested Streamlit app and companion files.
# It writes:
# - /mnt/data/app.py  (Streamlit dashboard)
# - /mnt/data/requirements.txt  (dependencies)
# - /mnt/data/.env.example  (environment variables template)
# - /mnt/data/README.txt  (quick start)
...
env_example = dedent('''
# Copy to .env or set in Streamlit Cloud as secrets.
KOTAK_BASE_URL=https://api.kotaksecurities.com
# Preferred simple path: paste your long-lived API token here (as provided by NEO dashboard)
KOTAK_API_TOKEN=xxxx-your-api-token-xxxx

# Optional (if you enable auto-login – adjust endpoints in code to Kotak's actual ones)
KOTAK_USERNAME=your_username
KOTAK_PASSWORD=your_password
KOTAK_CLIENT_CODE=your_client_code
KOTAK_TOTP_SECRET=YOUR_TOTP_BASE32_SECRET
...
with open("/mnt/data/.env.example", "w", encoding="utf-8") as f:
    f.write(env_example)

with open("/mnt/data/README.txt", "w", encoding="utf-8") as f:
    f.write(readme)

"/mnt/data files created: app.py, requirements.txt, .env.example, README.txt"
